...
def billing(link_dir): #link_dir = uploads_dir(flask)
    import datetime
    import time
    import sys
    import os
    ...
    file_list = os.listdir(path=link_dir)
    start_link = []

    for link in file_list:
        start_link.append(os.path.join(link_dir, link))
    ...
    if unknown_nubmers or reserve_number_charges:
        up_dict = dict(switch=0, unknown = unknown_nubmers,
                       reserve = reserve_number_charges)
        return up_dict
    ...
    up_dict = dict(switch=1, unknown = unknown_nubmers,
                   reserve=reserve_number_charges)
    return up_dict
